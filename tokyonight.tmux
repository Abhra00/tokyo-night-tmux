...
window_id_style="$(echo "$TMUX_VARS" | grep '@tokyonight-tmux_window_id_style' | cut -d" " -f2)"
pane_id_style="$(echo "$TMUX_VARS" | grep '@tokyonight-tmux_pane_id_style' | cut -d" " -f2)"
zoom_id_style="$(echo "$TMUX_VARS" | grep '@tokyonight-tmux_zoom_id_style' | cut -d" " -f2)"
github_status_enabled="$(echo "$TMUX_VARS" | grep '@tokyonight-tmux_github_status' | cut -d" " -f2)"
